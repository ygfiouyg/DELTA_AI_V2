...
const INTENT_PATTERNS: IntentPattern[] = [
  ...
  {
    type: 'summarize',
    weight: 4, // V.55: Higher than compile — "اجمعلي اهم النقاط" should be SUMMARIZE
    patterns: [
      ...
      // V.55: "اجمعلي اهم النقاط" = SUMMARIZE (not COMPILE)
      /اجمع\s*(لي?\s*)?(اهم|الاهم|النقاط|النقاط المهمة|اهم النقاط|اهم المعلومات)\s*/i,
      /اجمع\s*(لي?\s*)?(النقاط|المعلومات|الافكار|المفاهيم)\s*(ال)?(مهمة|الرئيسية|الاساسية)?\s*/i,
      /اجمعلي?\s*(اهم|الاهم)\s*/i,
      /جمع\s*(لي?\s*)?(اهم|الاهم)\s*(النقاط|المعلومات|الافكار)\s*/i,
...
- summarize: تلخيص المحتوى، جمع النقاط المهمة، استخراج الأفكار الرئيسية (مثل: "اجمعلي اهم النقاط"، "لخصلي"، "اعملي ملخص")
- compile: تجميع محاضرات متعددة في ملف واحد كامل (مثل: "اجمع كل المحاضرات في ملف"، "اعمل تجميعة شاملة")
