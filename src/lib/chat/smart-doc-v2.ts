...
async function routeSummarize(
  ...
  // V.56: EXACT replica of reference PDF structure
  // ────────────────────────────────────────────────
  // Page 1: Cover (generated by html-template-generator)
  // Page 2: Metadata — "الجزء 1 — ملف PDF الأصلي للمحاضرة: {filename} ({size})"
  //         + ref id + 5 color dots
  // Page 3: Key Insight callout (★) — executive summary
  //         + ⚡ per-file summary card (paragraph only, NO key points inline)
  // Page 4: 5 numbered key points (consolidated across all files)
  //         + 4 connecting themes (bullets)
  //
  // CRITICAL: Match reference EXACTLY — compact 4-page output, NOT 8 pages.
  let markdown = '';

  // ── Page 2: Metadata Section (matches reference exactly) ──
  // Format: "الجزء 1 — ملف PDF الأصلي للمحاضرة: {filename} ({size})"
  //         + [DELTA_PDF_REF:uuid:filename:size] tag
  //         + 5 color dots (rendered by template)
  const fileMeta = files.map((f, idx) => {
    const cleanName = sanitizeFileName(f.name);
    const sizeMb = f.content ? `(${(f.content.length / 1024 / 1024).toFixed(1)} MB)` : '';
    const partLabel = language === 'en' ? `Part ${idx + 1}` : `الجزء ${idx + 1}`;
    const pdfLabel = language === 'en' ? 'Original PDF file' : 'ملف PDF الأصلي للمحاضرة';
    return `# ${partLabel} — ${pdfLabel}: ${cleanName} ${sizeMb}`.trim();
  }).join('\n\n');
  markdown += fileMeta + '\n\n';

  // ── Page 3: Key Insight callout (★) — executive summary ──
  // Matches reference: ★ KEY INSIGHT header + ⚡ bold label + paragraph
  if (result.crossSummary) {
    const insightLabel = language === 'en' ? 'Executive Overview' : 'نظرة عامة تنفيذية';
  ...
  // ── Page 4: 5 Consolidated Key Points (numbered, matches reference) ──
  // Collect all key points across all files, dedupe, pick top 5 most important
  const allKeyPoints: string[] = [];
  const seen = new Set<string>();
  for (const fileSummary of result.perFile) {
    for (const point of fileSummary.keyPoints) {
      // Strip emoji prefixes from individual points (reference has clean numbered list)
      const cleaned = point.replace(/^(📌|💡|⚠️|🔗|⚖️|⭐|✅|❌|📖|🔬|🎯|⚡|★)\s*/, '').trim();
      const key = cleaned.toLowerCase().slice(0, 80);
      if (cleaned && !seen.has(key)) {
        seen.add(key);
        allKeyPoints.push(cleaned);
      }
    }
  }
  // Match reference EXACTLY: 5 key points (not 8)
  const focusedPoints = allKeyPoints.slice(0, 5);

  if (focusedPoints.length > 0) {
    const kpHeader = language === 'en' ? '🔑 Key Points' : '🔑 النقاط الجوهرية';
    markdown += `## ${kpHeader}\n\n`;
    for (let i = 0; i < focusedPoints.length; i++) {
      markdown += `**${i + 1}.** ${focusedPoints[i]}\n\n`;
    }
  }

  // ── Page 4 (bottom): 4 Connecting Themes (bullets, matches reference) ──
  // Reference shows exactly 4 themes with ● bullet markers
  if (result.commonThemes.length > 0) {
    const themesHeader = language === 'en' ? '🔗 Connecting Themes' : '🔗 المواضيع المترابطة';
    markdown += `## ${themesHeader}\n\n`;
    // Limit to 4 themes to match reference exactly
    const themes = result.commonThemes.slice(0, 4);
...
async function generatePDFFromContent(
  ...
  // V.51: Use the rendering-pipeline (the one that makes beautiful PDFs)
  // The Omni orchestrator generates raw LLM HTML without design system.
  // The rendering-pipeline uses:
  //   - generateDesignReasoning (AI picks colors, layout, fonts)
  //   - generateHTMLTemplate (rainbow strips, section headers, CSS Grid)
  //   - generateUniquePalette (unique colors per topic)
  //   - Playwright for PDF rendering
  // This is what made the beautiful PDFs the user liked before.

  onProgress?.('designing', 75, 'جاري تحليل التصميم واختيار الألوان...', 'AI design');
