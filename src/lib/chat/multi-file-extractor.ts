...
const PER_FILE_SUMMARY_PROMPT_AR: Record<string, string> = {
  brief: `أنت محرك تحليلي عميق. اقرأ المحتوى ونفذ خطوات التفكير التالية قبل الكتابة:

STEP 1 — Information Architecture:
- اشل كل الضوضاء: null bytes، أسماء ملفات خام، unicode غير مقروء
- استخرج المفاهيم الأساسية، المعادلات، الأرقام المهمة، والخطوات التحليلية
...
- استخدم :::callout-rule للقواعد والقوانين
- استخدم :::callout-error للتحذيرات والأخطاء الشائعة

أجب بصيغة JSON:
{
  "summary": "ملخص تنفيذي عميق — مش سرد، بل تحليل يربط الأفكار",
  "keyPoints": ["📌 مفهوم جوهري مع تعريف مختصر", "💡 نقطة مهمة مع السبب", "⚠️ خطأ شائع مع التصحيح"]
}`,

  medium: `أنت محرك تحليلي عميق. اقرأ المحتوى ونفذ 3 خطوات تفكير قبل الكتابة:

STEP 1 — Information Architecture (Semantic Extraction):
- اشل كل الضوضاء: null bytes (0000)، أسماء ملفات خام (Lec 2.pdf)، unicode غير مقروء
- استخرج: المفاهيم الأساسية، المعادلات، الأرقام المهمة، الخطوات التحليلية
...
2. حلل كل قسم: ما الفكرة المحورية؟ ما التفاصيل الداعمة؟
3. اربط الأفكار — كيف يبني الموضوع من البسيط للمعقد؟
4. فكر كطالب: ما اللي محتاج أعرفه للامتحان؟ ما اللي صعب ويحتاج تبسيط؟
5. فكر كدكتور: ما المفاهيم اللي لازم يفهمها الطالب مش بس يحفظها؟
...
أجب بصيغة JSON فقط (بدون markdown إضافي):
{
  ...
  "keyPoints": [
    ...
    "⚠️ خطأ شائع مع التصحيح",
    "🔗 علاقة بين فكرتين مع توضيح",
    "📖 مصطلح مع تعريفه المبسط"
  ]
}`,

  detailed: `أنت محرك تحليلي عميق — بروفيسور، طالب متفوق، ومصمم تعليمي. اقرأ المحتوى ونفذ 3 خطوات تفكير قبل الكتابة:
...
2. **التحليل**: فكك كل مفهوم لمكوناته. ما المبادئ؟ ما القواعد؟ ما الاستثناءات؟
3. **الربط**: كيف تترابط الأفكار؟ ارسم خريطة ذهنية — من البديهي للمعقد
4. **التقييم**: ما أهم نقطة؟ ما النقطة اللي معظم الطلاب بيغلطوا فيها؟
5. **التبسيط**: خد المفاهيم الصعبة وابسطها بأمثلة من الحياة اليومية
6. **التطبيق**: فكر في أسئلة امتحان محتملة — ما اللي ممكن يسأل عنه الدكتور؟
