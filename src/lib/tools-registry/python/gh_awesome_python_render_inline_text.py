"""
Tool: awesome-python_render_inline_text
...
def execute(children):
    """Execute render_inline_text from vinta/awesome-python."""
    try:
        import importlib
        try:
            mod = importlib.import_module("awesome_python")
            if hasattr(mod, "render_inline_text"):
                fn = getattr(mod, "render_inline_text")
                result = fn(children)
        ...
        return {
            ...
            "original_function": "render_inline_text",
            "docstring": "Extract plain text from a heading node.",
            "params": ["children"],
        }
