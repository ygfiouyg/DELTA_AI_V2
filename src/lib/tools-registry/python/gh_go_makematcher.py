...
def execute(klass):
    """Execute makematcher from golang/go."""
    try:
        import importlib
        try:
            mod = importlib.import_module("go")
            if hasattr(mod, "makematcher"):
                fn = getattr(mod, "makematcher")
                result = fn(klass)
...
if __name__ == "__main__":
    ...
    if "--args_file" in _sys.argv:
        ...
        _sys.exit(0)
    print(json.dumps({"usage": "Use --args_file <path> with JSON args", "params": ["klass"]}))
