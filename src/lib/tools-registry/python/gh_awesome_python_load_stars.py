"""
Tool: awesome-python_load_stars
...
def execute(path):
    """Execute load_stars from vinta/awesome-python."""
    try:
        import importlib
        try:
            mod = importlib.import_module("awesome_python")
            if hasattr(mod, "load_stars"):
                fn = getattr(mod, "load_stars")
                result = fn(path)
        ...
        return {
            ...
            "original_function": "load_stars",
            "docstring": "Sort entries by stars descending, then name ascending.      Three tiers: starred entries first, stdlib second, other non-starred last.",
            "params": ["path"],
        }
...
if __name__ == "__main__":
    ...
    if "--args_file" in _sys.argv:
        ...
        _sys.exit(0)
    print(json.dumps({"usage": "Use --args_file <path> with JSON args", "params": ["path"]}))
