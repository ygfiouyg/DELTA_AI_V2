...
def execute(ui, url, rev):
    """Execute golookup from golang/go."""
    try:
        import importlib
        try:
            mod = importlib.import_module("go")
            if hasattr(mod, "golookup"):
                fn = getattr(mod, "golookup")
                result = fn(ui, url, rev)
...
if __name__ == "__main__":
    ...
    if "--args_file" in _sys.argv:
        ...
        _sys.exit(0)
    print(json.dumps({"usage": "Use --args_file <path> with JSON args", "params": ["ui", "url", "rev"]}))
