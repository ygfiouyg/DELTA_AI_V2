"""
Tool: awesome-python_extract_github_repos
...
def execute(text):
    """Execute extract_github_repos from vinta/awesome-python."""
    try:
        import importlib
        try:
            mod = importlib.import_module("awesome_python")
            if hasattr(mod, "extract_github_repos"):
                fn = getattr(mod, "extract_github_repos")
                result = fn(text)
        ...
        return {
            ...
            "original_function": "extract_github_repos",
            "docstring": "Write the star cache to disk, creating data/ dir if needed.",
            "params": ["text"],
        }
...
if __name__ == "__main__":
    ...
    if "--args_file" in _sys.argv:
        ...
        _sys.exit(0)
    print(json.dumps({"usage": "Use --args_file <path> with JSON args", "params": ["text"]}))
