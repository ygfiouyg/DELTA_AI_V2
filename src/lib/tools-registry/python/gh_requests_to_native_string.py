"""
Tool: requests_to_native_string
...
def execute(string, encoding):
    """Execute to_native_string from psf/requests."""
    try:
        import importlib
        ...
        for submod_name in submodules_to_try:
            try:
                ...
                if hasattr(submod, "to_native_string"):
                    fn = getattr(submod, "to_native_string")
                    result = fn(string, encoding)
        ...
        try:
            ...
            if hasattr(mod, "to_native_string"):
                fn = getattr(mod, "to_native_string")
                result = fn(string, encoding)
        ...
        return {
            ...
            "original_function": "to_native_string",
            "docstring": "Determine if unicode string only contains ASCII characters.      :param str u_string: unicode string to check. Must be unicode         and not Python 2 `str`.     :rtype: bool",
...
if __name__ == "__main__":
    ...
    if "--args_file" in _sys.argv:
        ...
        _sys.exit(0)
    print(json.dumps({"usage": "Use --args_file <path> with JSON args", "params": ["string", "encoding"]}))
