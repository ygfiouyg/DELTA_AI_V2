...
export default function AuthPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/draix/auth');
      if (res.ok) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-display font-bold mb-2">🔐 Auth المصادقة</h1>
      ...
      <div className="draix-card">
        {loading ? (
          ...
          <div className="text-center py-12">
            <div className="text-4xl mb-3">🔐 Auth</div>
            <p className="text-draix-muted">No data available</p>
            <p className="text-xs text-draix-muted mt-2">Connected to: /api/draix/auth</p>
          </div>
        )}
      </div>
    </div>
