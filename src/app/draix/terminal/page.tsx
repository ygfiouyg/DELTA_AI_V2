...
export default function TerminalPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/draix/terminal');
      if (res.ok) {
        const result = await res.json();
        setData(Array.isArray(result) ? result : (result.data || []));
      }
    } catch (e) {
      console.error('Failed to fetch terminal:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-display font-bold mb-2">🧪 Terminal إعدادات الطرفية</h1>
      ...
      <div className="draix-card">
        {loading ? (
          ...
          <div className="text-center py-12">
            <div className="text-4xl mb-3">🧪 Terminal</div>
            <p className="text-draix-muted">No data available</p>
            <p className="text-xs text-draix-muted mt-2">Connected to: /api/draix/terminal</p>
          </div>
        )}
      </div>
    </div>
