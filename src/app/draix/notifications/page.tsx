...
export default function NotificationsPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/draix/notifications');
      if (res.ok) {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-display font-bold mb-2">🔔 Notifications الإشعارات</h1>
      ...
      <div className="draix-card">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-xs text-draix-muted mt-2">Connected to: /api/draix/notifications</p>
          </div>
        )}
      </div>
    </div>
